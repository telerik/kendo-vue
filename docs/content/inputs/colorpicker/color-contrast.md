...
description: "Use the Kendo UI for Vue Native ColorPicker to check the color contrast of two colors in Vue projects."
...
To enable the functionality, use the [`gradientSettings`]({% slug api_inputs_colorgradientprops %}) configuration and set its [`backgroundColor`]({% slug api_inputs_colorgradientprops %}#toc-backgroundColor) property.

The following example demonstrates how to display the Color Contrast tool in the ColorPicker gradient popup.

{% meta height:550 %}
{% embed_file colorpicker/colorcontrast/main.vue preview %}
{% embed_file colorpicker/colorcontrast/main.js %}
{% endmeta %}
