---
title: Appearance
description: 'Add alternative styling options to the Kendo UI for Vue SegmentedControl in Vue projects.'
...
Each item in the SegmentedControl accepts the [`svgIcon`](slug:api_buttons_segmenteditemprops#svgIcon) property to display an SVG icon. You can use icons from the [`@progress/kendo-svg-icons`](https://www.npmjs.com/package/@progress/kendo-svg-icons) package or define custom SVG icon objects. Icons can be combined with text, used alone (with a `title` for accessibility), or omitted for text-only items.

<demo metaUrl="buttons/segmentedcontrol/icons/" height="480"></demo>
