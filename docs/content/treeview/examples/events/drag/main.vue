...
function onItemDragOver(event) {
    events.value.unshift(`The dragOver event of Node ${event.item.text} was triggered`);
    dragOverCnt.value++;
    dragClue.value.show(event.pageY + 10, event.pageX, event.item.text, getClueIcon(event));
}

function onItemDragStart(event) {
    events.value.unshift(`The drag of Node ${event.item.text} has started`);
}
...
function onItemDragEnd(event) {
    ...
    events.value.unshift(`The drag of Node ${event.item.text} has ended`);
}
