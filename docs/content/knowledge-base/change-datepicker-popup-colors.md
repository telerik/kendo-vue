...
Use the [`popup`](slug:api_dateinputs_datepickerprops#toc-popup) prop to pass a [`custom`](slug:custom_rendering_datepicker#toc-customizing-the-popup) popup template to the DatePicker.

In the custom popup template we must use the [`Popup`](slug:overview_popup) component and its [`popupClass`](slug:api_popup_popupprops#toc-popupclass) property.
