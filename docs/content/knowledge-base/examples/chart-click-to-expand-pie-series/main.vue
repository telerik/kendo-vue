...
const labelContent = (props) => {
    const formattedNumber = Number(props.dataItem.value).toLocaleString(undefined, {
        style: 'percent',
        minimumFractionDigits: 2,
    });
    return `${props.dataItem.category} years old: ${formattedNumber}`;
};

const onSeriesClick = (event) => {
    pieData.value = pieData.value.map((item) => ({
        ...item,
        exploded: item === event.dataItem ? !item.exploded : false,
    }));
};
</script>
