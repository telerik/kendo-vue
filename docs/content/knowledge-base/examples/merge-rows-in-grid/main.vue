<template>
  <Grid :data-items="products" :columns="columns" :cell-render="myCellTemplate">
    <template #myTemplate="{ props }">
      <td
        :rowspan="2"
        v-if="props.field === 'Discontinued' && props.dataItem.ProductID % 2 !== 0"
      >
        {{ getNestedValue(props.field, props.dataItem) }}
      </td>
      <td v-else-if="props.field !== 'Discontinued'" :class="props.className">
        {{ getNestedValue(props.field, props.dataItem) }}
      </td>
    </template>
  </Grid>
...
const myCellTemplate = 'myTemplate';
