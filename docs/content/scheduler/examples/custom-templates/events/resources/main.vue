        ...
        <SchedulerItem v-bind="props">
          <div class="k-event-template" v-if="!props.isAllDay">
            ...
            <div :style="{ 'padding-top': '8px', display: 'flex' }">
              ...
              <span :style="{ 'padding-left': '5px' }">{{ props.description }}</span>
            </div>
          </div>
...
const views = [{ name: 'day' }, { name: 'month' }];
const resources = [
    {
        name: 'Rooms',
        data: [
...
function getOwnerResource(ownerID) {
    const resource = resources[1].data.find((el) => el.value === ownerID);
    return resource || '';
}
</script>
