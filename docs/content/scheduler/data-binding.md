---
title: Data Binding
description: "Get started with the Kendo UI for Vue Scheduler component, bind it to your data, and take advantage of the Kendo UI for Vue Native Scheduler built-in editing functionality."
...
The Kendo UI for Vue Native Scheduler component accepts an array of [`DataItem`]({% slug api_scheduler_scheduleritemprops %}#toc-dataitem) objects through the [`dataItems`]({% slug api_scheduler_schedulerprops %}#toc-dataitems) property.

## Getting Started

The `DataItem` type does not limit the user to a specific fields in the `data-items` array but expects that the [`modelFields`]({% slug api_scheduler_schedulerprops %}#toc-modelfields) property is set as well, or that the `DataItem` has the default fields mapping.
...
{% meta height:700 %}
{% embed_file data-binding/data/main.vue preview %}
{% embed_file data-binding/data/main.js %}
{% endmeta %}
...
{% meta height:700 %}
{% embed_file data-binding/model-fields/main.vue preview %}
{% embed_file data-binding/model-fields/main.js %}
{% endmeta %}
...
The following example demonstrates how to handle the `onDataChange` callback and update the local state. To trigger a data change, drag the slot in a different time slot. For more information, refer to the article on [editing the Kendo UI for Vue Native Scheduler]({% slug editing_scheduler %}).



{% meta height:770 %}
{% embed_file data-binding/data-change/main.vue preview %}
{% embed_file data-binding/data-change/main.js %}
{% endmeta %}
 -->

## Suggested Links

<!-- * [Editing in the Kendo UI for Vue Native Scheduler]({% slug editing_scheduler %}) -->
