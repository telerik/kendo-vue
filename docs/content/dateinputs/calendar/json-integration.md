...
description: "Bind the Kendo UI for Vue Calendar to dates which are serialized as strings."
...
3. Wire the [`change`]({% slug api_dateinputs_calendarprops %}#toc-onchange) event of the Calendar to get the selected `Date` value.

The following example demonstrates how to set the value of the Calendar.

{% meta height:430 %}
{% embed_file calendar/json/main.vue preview %}
{% embed_file calendar/json/main.js %}
{% endmeta %}
