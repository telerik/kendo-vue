...
description: 'Learn how to enable sticky (pinned) group headers and footers in the Kendo UI for Vue Data Grid so they remain visible while scrolling through grouped data.'
components: ['datagrid']
...
The Kendo UI for Vue Data Grid supports sticky (pinned) group header and footer rows that remain visible at the top or bottom of the scrollable area while the user scrolls through grouped data. This makes it easy to always see which group the current data belongs to and what the aggregate values are.
...
{% meta height:580 %}
{% embed_file sticky-group-headers/main.vue preview %}
{% embed_file sticky-group-headers/main.js %}
{% embed_file sticky-group-headers/gd-products.js %}
{% endmeta %}
