...
function update(data, item, remove) {
    ...
    if (index >= 0) {
        ...
        const maxId = data.reduce((max, p) => (p.ProductID > max ? p.ProductID : max), 0);
        const newId = maxId + 1;
        updated = { ...item, ProductID: newId };
...
function cancelChanges() {
    updatedData.value.forEach((item) => {
        if (item.inEdit) {
...
function getData() {
    const processed = process(updatedData.value, {
        take: take.value,
        skip: skip.value,
        sort: sort.value,
        filter: filter.value,
    });
    gridData.value = processed.data;
    total.value = process(updatedData.value, { filter: filter.value }).total;
}
