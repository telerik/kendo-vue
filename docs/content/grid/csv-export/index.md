---
title: Basics
description: "Get started with the Kendo UI for Vue Data Grid CSV export feature using the built-in @progress/kendo-csv library."
...
The [`csv`]({% slug api_grid_gridprops %}#toc-csv) property of the Grid component enables you to export the content of the Kendo UI for Vue Data Grid to CSV format using the built-in `@progress/kendo-csv` library and `@progress/kendo-file-saver` for file saving.
...
> **Note**: When using `:auto-process-data="true"`, the `allPages` option controls pagination behavior during export. By default `allPages` is `true`, which exports all data. Set `allPages: false` to export only the current page. See the [Basic Configuration Example](#basic-configuration-example) below for more details.
...
-   [Programmatic export (exportAsCsv & getCsvBlob)]({% slug programmaticexport_csvexport_grid %})
