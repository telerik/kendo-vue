...
1. Load the font icons either by [using the precompiled CSS](#toc-loading-icons-through-precompiled-css) or by [using a CDN link](#toc-loading-icons-through-a-cdn).
1. Add an empty `<span>` element that will hold the font icon.
1. Assign a `k-icon k-font-icon` CSS classes that are followed by a predefined class from the [list of font icons](#toc-list-of-font-icons) to an HTML tag. For example:
    ```html
